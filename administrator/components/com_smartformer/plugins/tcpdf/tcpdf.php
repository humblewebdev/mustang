...
if (!class_exists('TCPDF')) {
	 ...
	 */ 
	define('PDF_PRODUCER', 'TCPDF 4.6.011_PHP4 (http://www.tcpdf.org)');
	...
	*/
	class TCPDF {
		...
		*/
		 ...
		 */
		var $default_monospaced_font = 'courier';
		 ...
		 */
		var $cache_UTF8StringToArray = array();
		 ...
		 * @since 4.5.037 (2009-04-07)
		 */
		var $cache_maxsize_UTF8StringToArray = 8;
		 ...
		 * @since 4.5.037 (2009-04-07)
		 */
		var $cache_size_UTF8StringToArray = 0;
		...
		* @see getPageDimensions()
		*/
		function getBreakMargin($pagenum='') {
			if (empty($pagenum)) {
				return $this->bMargin;
			}
		...
		* @since 4.5.025
		*/
		function SetDefaultMonospacedFont($font) {
		...
		* @see Cell()
		*/
		function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $link='', $stretch=0, $ignore_min_height=false) {
		...
		* @since 4.5.037 (2009-04-07)
		*/
		function UniArrSubString($uniarr, $start='', $end='') {
			if (strlen($start) == 0) {
				$start = 0;
			}
			if (strlen($end) == 0) {
		...
		* @since 4.5.037 (2009-04-07)
		*/
		function UTF8ArrayToUniArray($ta) {
		...
		* @since 1.1
		*/
		function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false) {
			if ($x === '') {
				$x = $this->x;
			}
			if ($y === '') {
		...
		*/
		 ...
		 */
		function _destroy($destroyall=false, $preserve_objcopy=false) {
		...
		*/
		 ...
		 * @access public
		 */		
		function convertHTMLColorToDec($color='#FFFFFF') {
			global $webcolor;
			$returncolor = false;
		...
		*/
		 ...
		 * @since 2.4.000 (2008-03-06)
		*/
		function utf8Bidi($ta, $str='', $forcertl=false) {
		...
		* @since 2.1.002 (2008-02-12)
		*/
		function Bookmark($txt, $level=0, $y=-1, $page='') {
			if ($level < 0) {
				$level = 0;
			}
		...
		* @since 4.6.005 (2009-04-24)
		*/
		function setSignature($signing_cert='', $private_key='', $private_key_password='', $extracerts='', $cert_type=2, $info=array()) {
		...
		* @see getAliasNbPages(), PageNo(), Footer()
		*/
		function AliasNumPage($alias='{pnb}') {
		...
		* @see addTOC()
		*/
		function formatTOCPageNumber($num) {
			return number_format((float)$num, 0, '', '.');
		}
		...
		* @access public
		*/
		function ImageEps($file, $x='', $y='', $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0) {
			if ($x === '') {
				$x = $this->x;
			}
			if ($y === '') {
		 ...
		 * @access public
		 */
		function write2DBarcode($code, $type, $x='', $y='', $w='', $h='', $style='', $align='') {
			if ($this->empty_string($code)) {
				return;
			}
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		 ...
		 */
		function writeHTMLCell($w, $h, $x, $y, $html='', $border=0, $ln=0, $fill=0, $reseth=true, $align='', $autopadding=true) {
		...
		*/
        function getHTMLUnitToUnits($htmlval, $refsize=1, $defaultunit='px', $points=false) {
		...
		* @since 4.5.000 (2008-12-31)
		*/
		function writeDiskCache($filename, $data, $append=false) {
			if ($append) {
				$fmode = 'ab+';
			} else {
				$fmode = 'wb+';
		...
		* @since 4.5.000 (2008-12-31)
		*/
		function readDiskCache($filename) {
		...
		* @since 4.5.000 (2008-12-31)
		*/
		function setImageBuffer($image, $data) {
			if ($this->diskcache) {
		...
		* @since 4.5.000 (2008-12-31)
		*/
		function setImageSubBuffer($image, $key, $data) {
			if (!isset($this->images[$image])) {
		...
		* @since 4.5.000 (2009-01-02)
		*/
		function setFontBuffer($font, $data) {
			if ($this->diskcache) {
		...
		* @since 4.5.000 (2009-01-02)
		*/
		function setFontSubBuffer($font, $key, $data) {
			if (!isset($this->fonts[$font])) {
		...
		* @since 4.5.000 (2009-01-02)
		*/
		function addTOC($page='', $numbersfont='', $filler='.') {
