...
if (defined( '_JEXEC' )) $mainframe->registerEvent( 'onPrepareContent', 'plgContentSmartformer' );
	else $_MAMBOTS->registerFunction( 'onPrepareContent', 'botContentSmartformer' );
...
function plgContentSmartformer( &$row, &$params, $page=0 )
{
	...
	if (defined( '_JEXEC' )) {
		...
		$GLOBALS['sf_absolute_path']=substr(__FILE__,0,strpos(strtolower(__FILE__),'plugins')-1);
	}
...
function botContentSmartformer( $published, &$row, &$params, $page=0 )
{
	$pos=strpos($row->text,'{smartformer:');
	$pos2=strpos($row->text,'}');
	if ($pos===false || $pos2===false) return true;
