...
<div class="componentheading<?php echo $params->get( 'pageclass_sfx' ); ?>"><h1 class="ckformhead"><?php echo $form->title; ?></h1></div>

<script type="text/javascript">

window.addEvent('domready', function(){
	var myTips = new Tips('.ckform_tooltip<?php echo $form->id; ?>', {
		initialize:function(){
...
?>		
	...
	$('ckform<?php echo $form->id; ?>').onsubmit = function(event){
<?php 
	for ($i=0;$i < $nbFields; $i++)
	{ 
		$field = $fields[$i];
...
?>
	...
	var myForm = new FormCheck('ckform<?php echo $form->id; ?>', {
...
<?php } ?>
	<table class="contentpaneopen<?php echo $params->get( 'pageclass_sfx' ); ?>" id="ckformcontainer">

	<form action="index.php?option=com_ckforms&view=ckforms&task=send&id=<?php echo $form->id; ?>" method="post" name="ckform" id="ckform<?php echo $form->id; ?>" class="ckform  ckformMOD <?php echo $form->formCSSclass; ?>"<?php if($upload == true) { ?> enctype="multipart/form-data"<?php } ?>>
...
<?php 
	...
	<?php if ($field->custominfo != "" && $field->typefield == "textarea") 
    {

?>       
			<img class="ckform_tooltip<?php echo $form->id; ?> ckform_tooltipcss" src="<?php echo JURI::root(true).'/components/com_ckforms/'; ?>img/info.png" />
<?php
	}
?>       
        </label>
...
<?php
		...
		{
?>
		<input type="text" name="<?php echo $field->name; ?>" id="<?php echo $field->name; ?>" value="<?php if (empty($this->post) ==false) {echo $this->post[$field->name];} else {echo $field->t_initvalueT;} ?>" class="<?php echo $validationclass; ?> inputbox ckCSSinput <?php echo $field->fieldCSSclass; ?> calendarinput" maxlength="10" <?php if ($field->readonly == "1") {echo ' readonly="true"';} ?> />
<?php	
		}
		break; 		
...
?>
	<div class="captchaCont">
        ...
        <?php 	
			...
			{
				?> 
        		<img class="ckform_tooltip<?php echo $form->id; ?> ckform_tooltipcss" src="<?php echo JURI::root(true).'/components/com_ckforms/'; ?>img/info.png" title="<?php echo $form->captchacustominfo; ?>" />
				<?php
			}
		?>        
